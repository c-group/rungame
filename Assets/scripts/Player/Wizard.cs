...
public class Wizard : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && jumpCount == 0)
        {
            ...
            if (Input.GetKey(KeyCode.S) && anim.GetCurrentAnimatorStateInfo(0).IsName("Wizard_Jump") == true)
            {
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("Wizard_jump") == true && Input.GetKeyDown(KeyCode.Space) && jumpCount == 1)
        {
            // 落下速度をリセット
            rb2d.velocity = Vector2.zero;
            // (0,1)方向に瞬間的に力を加えて跳ねさせる
            ...
            if (Input.GetKey(KeyCode.S) && anim.GetCurrentAnimatorStateInfo(0).IsName("Wizard_jump2") == true)
            {
                anim.SetBool("Attack", true);
            }
        }
